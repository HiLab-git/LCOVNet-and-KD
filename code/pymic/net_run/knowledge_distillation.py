...
class KnowledgeDistillation(NetRunAgent):
    def __init__(self, config, stage = 'train'):
        super(KnowledgeDistillation, self).__init__(config, stage)
    ...
    def create_network(self):
        if(self.student is None):
            student_name = self.config['network']['student']
            if(student_name not in self.net_dict):
                raise ValueError("Undefined network {0:}".format(student_name))
            self.student = self.net_dict[student_name](self.config['network'])
            
        if(self.teacher is None):
            teacher_name = self.config['network']['teacher']
            if(teacher_name not in self.net_dict):
                raise ValueError("Undefined network {0:}".format(teacher_name))
            self.teacher = self.net_dict[teacher_name](self.config['network'])
        ...
        if(self.tensor_type == 'float'):
            ...
            self.student.double()
            self.teacher.double()
        param_number = sum(p.numel() for p in self.student.parameters() if p.requires_grad)
    ...
    def training(self):
        ...
        for it in range(iter_valid):
            ...
            with torch.no_grad():
                teacher_softlabel = self.teacher(inputs)

            loss_dice = self.get_loss_value(data, outputs[0:4], labels_prob)    
            loss_kd   = self.kd_loss(outputs, teacher_softlabel)
            loss = 0.8 * loss_dice + 0.2 * loss_kd
    ...
    def validation(self):
        ...
        with torch.no_grad():
            ...
            for data in validIter:
                ...
                outputs = self.inferer.run(self.student, inputs)
    ...
    def train_valid(self):
        ...
        if(len(device_ids) > 1):
            ...
            self.student = nn.DataParallel(self.student, device_ids = device_ids)
        ...
        ckpt_init_name_student = self.config['training'].get('model_state_dict_student', None)
        ckpt_init_name_teacher = self.config['training'].get('model_state_dict_teacher', None)
        ckpt_init_mode = self.config['training'].get('ckpt_init_mode', 0)
        ckpt_for_optm  = None
        if(ckpt_init_name_student is not None):
            checkpoint_student = torch.load(ckpt_init_name_student, map_location = self.device)
            pretrained_dict = checkpoint_student['state_dict']
            model_dict = self.student.module.state_dict() if (len(device_ids) > 1) else self.student.state_dict()
            ...
            for k in pretrained_dict:
                logging.info(k)
            if (len(device_ids) > 1):
                self.student.module.load_state_dict(pretrained_dict, strict = False)
            else:
        ...
        if(ckpt_init_name_teacher is not None):
            checkpoint_teacher = torch.load(ckpt_init_name_teacher, map_location = self.device)
            pretrained_dict = checkpoint_teacher['state_dict']
            model_dict = self.teacher.module.state_dict() if (len(device_ids) > 1) else self.teacher.state_dict()
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if \
                k in model_dict and tensor_shape_match(pretrained_dict[k], model_dict[k])}
            logging.info("Initializing the following parameters with pre-trained model")
            for k in pretrained_dict:
                logging.info(k)
            if (len(device_ids) > 1):
                self.teacher.module.load_state_dict(pretrained_dict, strict = False)
            else:
        ...
        for it in range(iter_start, iter_max, iter_valid):
            ...
            if(valid_scalars['avg_fg_dice'] > self.max_val_dice):
                ...
                if(len(device_ids) > 1):
                    self.best_model_wts = copy.deepcopy(self.student.module.state_dict())
                else:
                    self.best_model_wts = copy.deepcopy(self.student.state_dict())
            ...
            if ((self.glob_it in iter_save_list) or stop_now):
                save_dict = {'iteration': self.glob_it,
                             ...
                             'model_state_dict': self.student.module.state_dict() \
                                 if len(device_ids) > 1 else self.student.state_dict(),
    ...
    def infer(self):
            ...
            if(self.config['testing'].get('test_time_dropout', False)):
                def test_time_dropout(m):
                    if(type(m) == nn.Dropout):
                        ...
                        m.train()
                self.student.apply(test_time_dropout)
        ...
        with torch.no_grad():
            for data in self.test_loader:
                ...
                pred = self.inferer.run(self.student, images)
