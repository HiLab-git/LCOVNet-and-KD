...
class SegmentationAgent(NetRunAgent):
    ...
    def create_network(self):
        if(self.net is None):
            net_name = self.config['network']['student_net_type']
        ...
        student_ckpt_name = self.config['net']['student_ckpt_save_dir']
        checkpoint = torch.load(ckpt_name, map_location = device)
        self.net.load_state_dict(checkpoint['model_state_dict'])

        
        if(self.net is None):
            teacher_net_name = self.config['network']['teacher_net_type']
            if(teacher_net_name not in self.net_dict):
                raise ValueError("Undefined network {0:}".format(net_name))
            self.teacher_net = self.net_dict[net_name](self.config['network'])
        if(self.tensor_type == 'float'):
            self.teacher_net.float()
        else:
            self.teacher_net.double()
        teacher_ckpt_name = self.config['net']['teacher_ckpt_save_dir']
        checkpoint = torch.load(ckpt_name, map_location = device)
    ...
    def training(self):
        ...
        for it in range(iter_valid):
            ...
            loss = self.get_loss_value(data, outputs[0], labels_prob)
