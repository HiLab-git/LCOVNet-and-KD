...
from pymic.net_run.knowledge_distillation import KnowledgeDistillation

def get_segmentation_agent(config, sup_type):
    assert(sup_type in ['fully_sup', 'semi_sup', 'self_sup', 'weak_sup', 'noisy_label', 'knowledge_distillation'])
    ...
    elif(sup_type == 'knowledge_distillation'):
        logging.info("\n********** Knowledge Distillation Learning **********\n")
        agent = KnowledgeDistillation(config, 'train')
...
def main():
    ...
    if(task == 'cls' or task == 'cls_nexcl'):
        ...
        # print(task)
        # print(config['dataset'].get('supervise_type', 'fully_sup'))
        # input()
