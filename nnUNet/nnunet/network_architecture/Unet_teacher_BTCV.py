...
class UNet(SegmentationNetwork):
    def __init__(self, C_in=32, n_classes=9, m=1):
    ...
    def forward(self, x):
        ...
        segout1 = self.upscale_logits_ops[0](
            self.segout1(x6))
        ...
        segout2 = self.upscale_logits_ops[1](
            self.segout2(x7))

        x8 = self.up3(x7)
        x8 = torch.cat((x8, _x2), dim=1)
        x8 = self.Decode_block3(x8)
        segout3 = self.upscale_logits_ops[2](
            self.segout3(x8))

        x9 = self.up4(x8)
        x9 = torch.cat((x9, _x1), dim=1)
        x9 = self.Decode_block4(x9)
        segout4 = self.upscale_logits_ops[3](
            self.segout4(x9))

        # print(segout4.shape, segout3.shape, segout2.shape, segout1.shape)
        if (self.do_ds == True):
        ...
        # # 训练的时候
        # return tuple([segout4, segout3, segout2, segout1])
        # # return tuple([x9, x8, x7, x6]) # feature KD
        # # # 测试的时候
        # # return segout4


class UNet_Feature(UNet):
    def __init__(self, C_in=32, n_classes=17, m=1):
        ...
        #     self.ft_chns[3]*2, self.ft_chns[3])
        # self.segout1 = nn.Conv3d(
        #     self.ft_chns[3], self.n_class, kernel_size=3, padding=1)

        # self.up2 = UnetBlock_Up(self.ft_chns[3], self.ft_chns[2])
        # self.Decode_block2 = UnetBlock_Encode(
        #     self.ft_chns[2]*2, self.ft_chns[2])
        # self.segout2 = nn.Conv3d(
        #     self.ft_chns[2], self.n_class, kernel_size=3, padding=1)

        # self.up3 = UnetBlock_Up(self.ft_chns[2], self.ft_chns[1])
        # self.Decode_block3 = UnetBlock_Encode(
        #     self.ft_chns[1]*2, self.ft_chns[1])
        # self.segout3 = nn.Conv3d(
        #     self.ft_chns[1], self.n_class, kernel_size=3, padding=1)
