...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, PL_output=False):
        l = []
        skips = []
        seg_outputs = []
        ...
        for u in range(len(self.tu)):
            ...
            if u+1 == len(self.tu):
                Penultimate_Layer_output = self.seg_outputs[u](x)
            l.append(x)
            seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x)))
        k = []
        k.append(seg_outputs[3])
        k.append(seg_outputs[2])
        k.append(seg_outputs[1])
        k.append(seg_outputs[0])
        k.append(l[3])
        k.append(l[2])
        k.append(l[1])
        k.append(l[0])
...
class Generic_UNet_Teacher(SegmentationNetwork):
    DEFAULT_BATCH_SIZE_3D = 2
    DEFAULT_PATCH_SIZE_3D = (64, 192, 160)
    SPACING_FACTOR_BETWEEN_STAGES = 2
    BASE_NUM_FEATURES_3D = 30
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        Questions? -> f.isensee@dkfz.de
        """
        super(Generic_UNet_Teacher, self).__init__()
        self.convolutional_upsampling = convolutional_upsampling
        self.convolutional_pooling = convolutional_pooling
        self.upscale_logits = upscale_logits
        if nonlin_kwargs is None:
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            ...
            # print(self.seg_outputs[u](x).shape)
        f = x
        # input('ana')
        # if self._deep_supervision and self.do_ds:
        #     return tuple([seg_outputs[-1]] + [i(j) for i, j in
        #                                       zip(list(self.upscale_logits_ops)[::-1], seg_outputs[:-1][::-1])])
        return f, seg_outputs[-1]
